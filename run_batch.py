...
def audio_to_list(fp, verbose_indent=0):
    ...
    cmd=[
            'ffmpeg', '-i', fp,
            #'-to', '30', #uncomment for testing mode
            '-f', 's16le',
            '-acodec', 'pcm_s16le',
            '-ac', '2',
            '-ar', '44100',
...
song_path=js['song_path']#r'I:\AllKemonoFriendsMusic\Official Releases\PPP In The Sky (FLAC)\05 - Gentoo Penguin - Hello！アイドル.flac'
...
try:
    os.mkdir(os.path.join('final_outputs',js['song_name']))
except:
    pass
...
for batch in js['isolations']:
    ...
    list_to_audio(ensembled,os.path.join('final_outputs',js['song_name'],js['song_name']+' (Offset '+str(offset)+', Ensembled - '+codename+').flac'))
...
if verbose: print('> ensembling EVERYTHING together...')
files=[os.path.join('final_outputs',js['song_name'],i) for i in os.listdir(os.path.join('final_outputs',js['song_name']))]
files_=[]
for i in files:
    if ', Ensembled' in i:
        files_.append(i)
files=['"'+i+'"' for i in files_]
ensemble_weights=['1']*len(files)
ensemble_weights=' '.join(ensemble_weights)
#note to self: numpy 1.26.3 was replaced with 1.23.5 to fix librosa error
os.system('python "'+mvsep_path.replace('inference','ensemble')+'" --files '+(' '.join(files))+' --weights '+ensemble_weights+' --type max_fft --output "'+os.path.join('final_outputs',js['song_name'],'ENSEMBLE.wav')+'"')